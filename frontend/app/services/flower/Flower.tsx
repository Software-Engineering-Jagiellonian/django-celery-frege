...
const Flower = () => {
  ...
  const TaskPicker = () => (
    <select className={styles.selectDropdown} onChange={handleChange} value={selected}>
      {flowerServices.map((el, i) => (
        <option className={styles.selectOption} key={i} value={el.id}>
          {el.name}
        </option>
      ))}
    </select>
