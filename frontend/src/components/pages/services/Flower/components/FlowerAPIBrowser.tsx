...
const FlowerAPIBrowser = () => {
    ...
    <div>
        ...
        <ul>
            {baseRequests.map((request: string) => <li key={request} onClick={() => getData(request)} className={styles.subLink}>{request}</li>)}
        </ul>
        ...
        <div className={styles.list}>
            {Object.entries(data).map(([key, value]: [string, any]) => <Sublist key={key} property={key} obj={value} />)}
        </div>
    </div>
    )
}
