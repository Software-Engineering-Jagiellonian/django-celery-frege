...
const Django = () => {
    ...
    <div>
        ...
        {error !== "" && <div className={styles.errorMessage}>Please remember you need to be logged into Django to use this feature <a href={`http://${targetDjango}:${portDjango}`}>Log in</a>. {error}</div>}
        <ul className={styles.list}>
            {Object.entries(data.data).map(([key, value]: [string, any]) => <Sublist key={key} property={key} obj={value} setData={setData} />)}
        </ul>
    </div>
    )
}
