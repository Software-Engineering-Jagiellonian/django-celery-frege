...
const PresentationMode = () => {
  ...
  const [dashboardId, setDashboardId] = useState<string>(localStorage.getItem(savedFullscreenDashboardId) || '')
  ...
  const saveConfig = () => {
    if (chosenDashboard === '' || chosenDashboard === "---") {setSelectAtLeastOne(true); return}
  ...
  return (
    <>
      <div className={styles.optionsRow}>
        <StyledButton className={styles.saveButton} onClick={saveConfig}>Save selection</StyledButton>

        <select className={styles.selectDropdown} onChange={e => setChosenDashboard(e.target.value)} value={chosenDashboard}>
          <option className={styles.selectOption} value={''}>---</option>
          {structure.sort((x, y) => x.name.localeCompare(y.name)).map((el: UnwrappedItemDTO) => <option key={el.id} className={styles.selectOption} value={el.id}>{el.name}</option>)}
        </select>
      </div>
