...
const Flower = () => {
    ...
    const typeSet = Array.from(new Set(logs.map(log => log.name)).add('---')).filter(el => el !== undefined && el !== null)
    const stateSet =  Array.from(new Set(logs.map(log => log.state)).add('---')).filter(el => el !== undefined && el !== null)
    ...
    <div>
        <div className={styles.optionsRow}>
            <div>
                ...
                <select className={styles.selectDropdown} onChange={e => setFilterType(e.target.value)} value={filterType}>
                    {typeSet.map(el => <option className={styles.selectOption} key={el} value={el}>{el}</option>)}
                </select>
            </div>

            <div>
                <h6>Filter by state</h6>
                <select className={styles.selectDropdown} onChange={e => setFilterState(e.target.value)} value={filterState}>
                    {stateSet.map(el => <option className={styles.selectOption} key={el} value={el}>{el}</option>)}
                </select>
            </div>
