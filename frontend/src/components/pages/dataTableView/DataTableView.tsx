...
const getMetricsCellRepr = (metricsString: string) => {
    ...
    return(<div className={styles.metricsCell}>
    ...
    <div>
        <div className={styles.languageOptionsRow}>
            ...
            <div className={styles.languageRow}>
                {languagesSet.map((el, i) => <div className={languages.some(lan => lan === el) ? styles.languageSelected : styles.languageNotSelected} key={i} onClick={() => handleLanguageClick(el)}>{el}</div>)}
            </div>
        </div>
        ...
        <div className={styles.optionsRow}>
            <div>
                ...
                <select className={styles.selectDropdown} onChange={handleStateClick} value={analyzed}>
                    {analyzedStatus.map((el, i) => <option className={styles.selectOption} key={i} value={el.value}>{el.name}</option>)}
                </select>
            </div>

            <div>
                <h6>Goto Page</h6>
                <input type="number" className={styles.pageSelector} value={page} onChange={handlePageClick} />
            </div>
        </div>
