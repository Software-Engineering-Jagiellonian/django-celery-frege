    ...
    case 'string':
      if (obj.startsWith('http'))
        return (
          <>
            <div>{property}</div>
            <div onClick={() => setData({ request: obj, data: {} })} className={styles.subLink}>
              {obj}
            </div>
          </>
        );
...
const SubObject: React.FC<{ property: string; obj: any; setData: any }> = ({
  property,
  obj,
  setData
}) => {
    ...
    return (
      <ul>
        {obj.map((el: any, i: number) => (
          <React.Fragment key={i}>{handleSubListDataTypes(property, el, setData)}</React.Fragment>
        ))}
      </ul>
    );
